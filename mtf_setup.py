...
변경사항:
  ...
  v3.0.3  타임프레임 정렬 카운트 기반 등급 강화
          B등급 score≥55 + aligned≥2 조건 추가
          A등급 score≥70 + 4h or 1h GC 조건 추가
          S등급 score≥85 + 일봉GC + (4h or 1h)GC 조건 추가
          4h K≥70 → timing_warning 플래그
          1h K≥80 → overbought_warning 플래그
─────────────────────────────────────────────
"""
...
def _stoch_rsi_k(closes: np.ndarray, rsi_p: int, stoch_p: int, k_smooth: int) -> np.ndarray:
    ...
    for i in range(stoch_p - 1, len(closes)):
        ...
        raw_k[i] = 50.0 if hi == lo else (rsi_vals[i] - lo) / (hi - lo) * 100
...
def _count_aligned_timeframes(d_long, d_mid, d_short, h4_short, h1_short) -> int:
...
def _summarize(mtf: dict) -> dict:
    ...
    aligned = _count_aligned_timeframes(d_long, d_mid, d_short, h4_short, h1_short)

    # ── 타이밍 경고 플래그 ──────────────────────────
    h4_k  = h4_short.get('k', 50.0)
    h1_k  = h1_short.get('k', 50.0)
    timing_warning    = h4_k >= 70   # 4h 단기 K ≥ 70
    overbought_warning = h1_k >= 80  # 1h 단기 K ≥ 80

    # ── 등급 판정 (타임프레임 정렬 + 점수 복합) ──────
    if any_buy_no:
        grade = 'X'
...
def btc_ma20_signal(daily_closes: list, weekly_closes: list) -> dict:
    """일봉 MA20, 주봉 MA20 동시 반환 (KRW 기준, USD는 dashboard에서 환산)"""
    result = {
...
def calc_relative_strength(coin_closes: list, btc_closes: list, period: int = 14) -> dict:
    ...
    grade = 'S' if rs >= 5 else ('A' if rs >= 2 else ('B' if rs >= 0 else ('C' if rs >= -3 else '-')))
    return {'rs': rs, 'grade': grade}


if __name__ == '__main__':
    print(f'mtf_setup.py {VERSION} 로드 완료 ✅')
    print(f'  등급기준: S(score≥85+일봉GC+4h/1hGC) A(score≥70+4h/1hGC) B(score≥55+aligned≥2) C(제외)')
    print(f'  타이밍경고: 4h K≥70 → timing_warning | 1h K≥80 → overbought_warning')
