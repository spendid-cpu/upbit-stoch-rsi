# -*- coding: utf-8 -*-
"""
dashboard.py — Upbit MTF 스캐너 Flask 대시보드 (v1.3)
변경사항:
  - 활성 트레이드 (TP/SL/타임아웃) 섹션 추가
  - 승률 / 통계 카드 추가
  - 트레이드 히스토리 페이지 (/trades)
"""
...
.card .sub{font-size:.66rem;color:var(--dim);margin-top:3px}
...
.sec-title{font-size:.92rem;font-weight:700;margin:18px 0 9px;color:#aaa}
...
.badge{display:inline-block;padding:2px 6px;border-radius:3px;font-size:.65rem;font-weight:700;margin-left:3px}
...
.b-chart  {background:#2a2020;color:#ffcc02;cursor:pointer;border:none;
           font-size:.65rem;font-weight:700;padding:2px 6px;border-radius:3px}
.macro-box{background:#131720;border:1px solid var(--border);border-radius:8px;
           padding:11px 13px;margin-bottom:16px}
.macro-box .title{font-size:.73rem;color:#666;margin-bottom:9px}

/* TP/SL 진행바 */
.progress-wrap{background:#252836;border-radius:3px;height:5px;margin-top:4px;overflow:hidden}
...
.chart-tabs{display:flex;gap:7px;margin-bottom:11px;flex-wrap:wrap}
.chart-tab{padding:4px 13px;border-radius:5px;border:1px solid var(--border);
           background:var(--bg);color:var(--muted);cursor:pointer;font-size:.76rem;transition:.15s}
...
/* 승률 바 */
.stat-bar-wrap{background:#252836;border-radius:4px;height:8px;margin-top:6px;overflow:hidden;display:flex}
.stat-win{background:var(--green)}
.stat-loss{background:var(--red)}
.stat-timeout{background:var(--yellow)}
...
<div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(140px,1fr))">
  <div class="card" style="border-color:#2e7d32">
    ...
    <div class="stat-bar-wrap" style="margin-top:6px">
      <div class="stat-win"    style="width:{{ (stats.win / total * 100)|round }}%"></div>
      <div class="stat-loss"   style="width:{{ (stats.loss / total * 100)|round }}%"></div>
      <div class="stat-timeout"style="width:{{ (stats.timeout / total * 100)|round }}%"></div>
    </div>
    {% endif %}
  </div>
  <div class="card">
...
<table>
  <thead>
    <tr>
      <th>티커</th><th>진입가</th><th>현재가</th>
      <th>PnL</th><th>TP 목표</th><th>SL 라인</th>
      <th>경과</th><th>타임아웃</th><th>진행</th>
    </tr>
  </thead>
  <tbody>
    ...
    {% set timeout_pct = [held / timeout_h * 100, 100] | min %}
    <tr>
      <td>
      ...
      <td style="color:var(--muted)">
        {{ '{:,.0f}'.format(ep) if ep >= 100 else '{:.4f}'.format(ep) }}
      </td>
      ...
      <td style="color:var(--green);font-size:.78rem">
        {% set tp = trade.tp_price or 0 %}
        {{ '{:,.0f}'.format(tp) if tp >= 100 else '{:.4f}'.format(tp) }}
      </td>
      <td style="color:var(--red);font-size:.78rem">
        {% set sl = trade.sl_price or 0 %}
        {{ '{:,.0f}'.format(sl) if sl >= 100 else '{:.4f}'.format(sl) }}
      </td>
      <td style="color:var(--muted);font-size:.76rem">{{ '%.1f'|format(held) }}h</td>
      <td style="font-size:.72rem;color:var(--dim)">{{ trade.timeout_at[11:16] if trade.timeout_at else '—' }}</td>
      <td style="min-width:70px">
        <!-- PnL 진행바: SL(-3%) ~ TP(+5%) 사이 -->
        {% set bar_pct = [(pnl + sl_pct) / (tp_pct + sl_pct) * 100, 0] | max %}
        {% set bar_pct = [bar_pct, 100] | min %}
        <div class="progress-wrap">
          <div class="progress-fill {% if pnl >= 0 %}fill-green{% else %}fill-red{% endif %}"
               style="width:{{ bar_pct|round }}%"></div>
        </div>
        <div style="font-size:.62rem;color:#555;text-align:center;margin-top:1px">
          SL ←&nbsp;&nbsp;&nbsp;→ TP
        </div>
      </td>
    </tr>
...
<div class="sec-title">📌 청산 완료 (이번 스캔)</div>
<div class="tbl-wrap">
<table>
  <thead>
    <tr><th>티커</th><th>결과</th><th>진입가</th><th>청산가</th><th>PnL</th><th>보유시간</th></tr>
  </thead>
  <tbody>
    ...
    <tr>
      ...
      <td>
        {% if t.result == 'TP' %}<span class="badge b-tp">🎯 TP</span>
        {% elif t.result == 'SL' %}<span class="badge b-sl">🛑 SL</span>
        {% else %}<span class="badge b-timeout">⏰ TIMEOUT</span>{% endif %}
      </td>
      <td style="color:var(--muted);font-size:.78rem">
        {% set ep = t.entry_price or 0 %}
        {{ '{:,.0f}'.format(ep) if ep >= 100 else '{:.4f}'.format(ep) }}
      </td>
      <td style="font-size:.78rem">
        {% set xp = t.exit_price or 0 %}
        {{ '{:,.0f}'.format(xp) if xp >= 100 else '{:.4f}'.format(xp) }}
      </td>
      <td>
        {% set pnl = t.pnl_pct or 0 %}
        <span class="pnl-bar {% if pnl>0 %}pnl-pos{% elif pnl<0 %}pnl-neg{% else %}pnl-neu{% endif %}">
          {{ '%+.2f'|format(pnl) }}%
        </span>
      </td>
      <td style="color:var(--muted);font-size:.76rem">{{ '%.1f'|format(t.hours_held or 0) }}h</td>
    </tr>
    {% endfor %}
  </tbody>
</table>
...
<table>
  <thead>
    <tr><th>티커</th><th>일봉K</th><th>등록가</th><th>현재가</th><th>등락률</th><th>수익률</th><th>차트</th><th>등록일</th></tr>
  </thead>
  <tbody>
    ...
    <tr>
      <td>
        ...
        <a href="https://upbit.com/exchange?code=CRIX.UPBIT.{{ item.ticker }}"
           target="_blank" style="color:#555;font-size:.65rem;margin-left:3px">↗</a>
      </td>
      ...
      <td style="color:var(--dim);font-size:.7rem">{{ item.registered_at[:10] }}</td>
    </tr>
    {% endfor %}
  </tbody>
</table>
...
<table>
  <thead>
    <tr><th>티커</th><th>4h K</th><th>1h K</th><th>트리거</th><th>진입가</th><th>TP</th><th>SL</th></tr>
  </thead>
  <tbody>
    {% for sig in state.entry_signals %}
    {% set tr = sig.trigger %}
    {% set ep = sig.entry_price %}
    <tr>
      ...
      <td style="font-size:.74rem">{{ tr.h1_trigger_type }}</td>
      <td style="color:var(--muted);font-size:.78rem">
        {% if ep %}{{ '{:,.0f}'.format(ep) if ep >= 100 else '{:.4f}'.format(ep) }}{% else %}—{% endif %}
      </td>
      <td style="color:var(--green);font-size:.78rem">
        {% if ep %}{{ '{:,.0f}'.format(ep*(1+tp_pct/100)) if ep >= 100 else '{:.4f}'.format(ep*(1+tp_pct/100)) }}{% else %}—{% endif %}
      </td>
      <td style="color:var(--red);font-size:.78rem">
        {% if ep %}{{ '{:,.0f}'.format(ep*(1-sl_pct/100)) if ep >= 100 else '{:.4f}'.format(ep*(1-sl_pct/100)) }}{% else %}—{% endif %}
      </td>
    </tr>
    {% endfor %}
  </tbody>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style="font-size:.76rem">{{ r.removal_type }}</td>
...
<div style="background:var(--surface);padding:11px;border-radius:6px;color:var(--red);font-size:.8rem">
  {{ state.error }}
</div>
{% endif %}
...
<div style="display:flex;justify-content:space-between;align-items:center;
  ...
  <div style="display:flex;gap:14px">
    <a href="/history" style="color:#64b5f6;font-size:.78rem;text-decoration:none">📜 신호 히스토리</a>
    <a href="/trades"  style="color:#ffcc02;font-size:.78rem;text-decoration:none">📊 트레이드 히스토리</a>
  </div>
  <span style="font-size:.7rem;color:var(--dim)">MTF Scanner v1.3 | 60s 자동갱신</span>
</div>
</div>
...
<div class="modal-bg" id="chartModal">
  <div class="modal">
    ...
    <div style="font-size:.68rem;color:var(--dim);margin-top:7px">
...
function renderChart(){
  ...
  chartInstance=new Chart(document.getElementById('stochChart').getContext('2d'),{
    ...
    data:{labels:kArr.map((_,i)=>i+1),datasets:[
      {label:'K',data:kArr,borderColor:'#ffcc02',backgroundColor:'transparent',
       borderWidth:1.5,pointRadius:0,tension:0.3},
      {label:'D',data:dArr,borderColor:'#64b5f6',backgroundColor:'transparent',
       borderWidth:1.5,pointRadius:0,tension:0.3},
    ]},
    options:{
      ...
      scales:{
        x:{display:false},
        y:{min:0,max:100,ticks:{color:'#666',font:{size:10}},grid:{color:'#1e2130'}}
      },
      plugins:{
    ...
    plugins:[{id:'refLines',beforeDraw(c){
      const{ctx,scales:{y}}=c;
      [{v:20,col:'rgba(76,175,80,.45)'},{v:80,col:'rgba(244,67,54,.45)'}].forEach(({v,col})=>{
...
table{width:100%;border-collapse:collapse;background:#1a1d27;min-width:460px}
...
.b-s{background:#1b3a1b;color:#66bb6a}
...
<div class="container">
  ...
  <div class="tw"><table>
    ...
    <tbody>
    {% for sig in history|reverse %}{% set t=sig.trigger %}
    <tr>
      ...
      <td style="color:#666;font-size:.75rem">
...
.card .lbl{font-size:.68rem;color:#888;margin-bottom:4px}
.card .val{font-size:1.2rem;font-weight:700}
.c-g{color:#4caf50}.c-r{color:#f44336}.c-y{color:#ffcc02}.c-b{color:#90caf9}
.sec{font-size:.9rem;font-weight:700;margin:14px 0 9px;color:#aaa}
.tw{overflow-x:auto;border-radius:8px}
table{width:100%;border-collapse:collapse;background:#1a1d27;min-width:500px}
th{background:#252836;padding:8px 11px;text-align:left;font-size:.7rem;color:#888}
td{padding:8px 11px;font-size:.78rem;border-top:1px solid #2a2d3a}
.badge{display:inline-block;padding:2px 6px;border-radius:3px;font-size:.65rem;font-weight:700}
.b-tp{background:#1b3a1b;color:#66bb6a}
.b-sl{background:#3a1f1f;color:#ef9a9a}
.b-to{background:#2a2010;color:#ffcc02}
.pnl-p{color:#4caf50;font-weight:700} .pnl-n{color:#f44336;font-weight:700}
.empty{color:#555;padding:20px 0;text-align:center}
.bar-wrap{background:#252836;border-radius:3px;height:6px;margin-top:6px;overflow:hidden;display:flex}
...
<div class="stat-grid">
  <div class="card" style="border-color:#2e7d32">
    ...
    <div class="val c-g">{{ '%.1f'|format(stats.win_rate or 0) }}%</div>
    {% set t=stats.total %}
    <div class="bar-wrap">
      <div class="bw" style="width:{{ (stats.win/t*100)|round }}%"></div>
      <div class="bl" style="width:{{ (stats.loss/t*100)|round }}%"></div>
      <div class="bt" style="width:{{ (stats.timeout/t*100)|round }}%"></div>
    </div>
  </div>
  <div class="card">
    <div class="lbl">총 거래 / 승 / 패 / 타임아웃</div>
    <div class="val c-b">{{ stats.total }}건</div>
    <div style="font-size:.72rem;margin-top:4px">
      <span class="c-g">{{ stats.win }}승</span> /
      <span class="c-r">{{ stats.loss }}패</span> /
      <span class="c-y">{{ stats.timeout }}타임아웃</span>
    </div>
  </div>
  <div class="card">
    <div class="lbl">평균 PnL</div>
    <div class="val {% if (stats.avg_pnl or 0)>=0 %}c-g{% else %}c-r{% endif %}">
      {{ '%+.2f'|format(stats.avg_pnl or 0) }}%
    </div>
  </div>
  <div class="card">
    ...
    <div class="val" style="font-size:.95rem">
      <span class="c-g">{{ '%+.2f'|format(stats.best_pnl or 0) }}%</span> /
      <span class="c-r">{{ '%+.2f'|format(stats.worst_pnl or 0) }}%</span>
    </div>
  </div>
</div>
{% endif %}
...
<div class="tw"><table>
  <thead>
    <tr><th>#</th><th>티커</th><th>결과</th><th>진입가</th><th>청산가</th>
        <th>PnL</th><th>보유시간</th><th>청산일시</th></tr>
  </thead>
  <tbody>
  ...
  <tr>
    ...
    <td>
      {% if t.result=='TP' %}<span class="badge b-tp">🎯 TP</span>
      {% elif t.result=='SL' %}<span class="badge b-sl">🛑 SL</span>
      {% else %}<span class="badge b-to">⏰ TIMEOUT</span>{% endif %}
    </td>
    <td style="color:#666;font-size:.75rem">
      {% set ep=t.entry_price or 0 %}{{ '{:,.0f}'.format(ep) if ep>=100 else '{:.4f}'.format(ep) }}
    </td>
    <td style="font-size:.75rem">
      {% set xp=t.exit_price or 0 %}{{ '{:,.0f}'.format(xp) if xp>=100 else '{:.4f}'.format(xp) }}
    </td>
    <td class="{% if (t.pnl_pct or 0)>=0 %}pnl-p{% else %}pnl-n{% endif %}">
...
@app.route('/trades')
def trades():
    hist  = scanner.load_trade_history()
    stats = scanner.calc_trade_stats()
    return render_template_string(TRADES_HTML, history=hist, stats=stats)
